...
class timeit:
    ...
    def __exit__(this,a,b,c):
        print("Runtime: {:.3f}s".format(time.time()-this.s))
...
def main():
    ...
    # optimum = {'n_estimators':450,'max_features':0.9,'min_samples_split':10}
    param_grid = [{'n_estimators':[_ for _ in range(50,451,100)],
    ...
    params = np.array(["n_estimators","max_features","min_samples_split"])
    for param in range(len(params)):
        cur_param = params[param]
        others = params[np.where(params!=params[param])]
        # code here
        print(f"training classifiers for {cur_param}")
        val_acc  = []
        test_acc = []
        for o in others:
            setattr(clf,o,optimum[o])
        for idx in range(len(param_grid[0][cur_param])):
            print(cur_param,"=",param_grid[0][cur_param][idx])
            setattr(clf,cur_param,param_grid[0][cur_param][idx])
            clf.fit(xdf_train_oh.values, ydf_train.values)
            y_hat = clf.predict(xdf_val_oh.values)
            y_tru = ydf_val.values
            val_acc.append(np.sum(y_hat==y_tru)/y_tru.shape[0])
            y_hat = clf.predict(xdf_test_oh.values)
            y_tru = ydf_test.values
            test_acc.append(np.sum(y_hat==y_tru)/y_tru.shape[0])
        fig=plt.figure()
        plt.plot(param_grid[0][cur_param],val_acc,'C0--',label='validation')
        plt.plot(param_grid[0][cur_param],test_acc,'r--',label='test')
        plt.xlabel("{} values".format(cur_param))
        plt.ylabel("Accuracy")
        plt.legend()
        plt.savefig('sensitivity_{}'.format(cur_param))
        plt.close(fig)
    ...
    # with open("best_model_{}.pkl".format(ts),"wb") as f:
    #     pickle.dump(best_model, f)
    # with open("best_params_{}.pkl".format(ts),"wb") as f:
